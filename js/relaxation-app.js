...
(function () {
  ...
  function renderDaySelector() {
    ...
    RELAXATION_DAYS.forEach((dayData) => {
      ...
      const progressPct = relaxProgress.getDayProgress(
        dayData.day,
        dayData.tasks.length,
      );
      const isComplete = relaxProgress.isDayComplete(dayData.day);
      // Extract description from title (e.g., "Day 1: Foundation" -> "Foundation")
      const description = dayData.title.split(": ")[1] || "";
  ...
  function renderTaskList() {
    ...
    relaxSession.dayData.tasks.forEach((task, index) => {
      ...
      const isComplete = relaxProgress.isTaskComplete(
        relaxSession.getDayNumber(),
        task.id,
      );
      if (isComplete) {
  ...
  function advanceToNextTask() {
    ...
    if (nextTask) {
      relaxProgress.setLastTaskIndex(
        relaxSession.getDayNumber(),
        relaxSession.currentTaskIndex,
      );
  ...
  function cacheElements() {
    ...
    elements.taskInstruction = document.getElementById(
      "relax-task-instruction",
    );
  ...
  function attachEventListeners() {
    ...
    elements.tipsBack.addEventListener("click", () =>
      showScreen("relax-day-screen"),
    );
